...
function getCategory() {
  return (
    document.querySelector('input[name="fCategory"]:checked')?.value || 'game'
  )
}
...
function updateLivePreview() {
  ...
  if (cat === 'anime') {
    ...
    badge.className = 'preview-category-badge anime'
  } else {
    badge.textContent = '🎮 Jogo'
    badge.className = 'preview-category-badge game'
  }
  ...
  if (banner) {
    ...
    wrap.innerHTML = `<div class="preview-banner-placeholder">sem banner</div>`
  }

  const html2 = sections
    .map(s => {
      if (s.type === 'paragraph') return s.text ? `<p>${s.text}</p>` : ''
      if (s.type === 'quote')
        return s.text ? `<blockquote>"${s.text}"</blockquote>` : ''
      if (s.type === 'image') return s.url ? `<img src="${s.url}" alt="">` : ''
      if (s.type === 'video')
        return `<div class="preview-video-ph">🎥 ${
          s.url || 'URL do vídeo'
        }</div>`
      return ''
    })
  ...
  $('pContent').innerHTML =
    html2 || '<p class="preview-empty-hint">conteúdo aparece aqui…</p>'
}
...
document
  .querySelectorAll('input[name="fCategory"]')
  .forEach(r => r.addEventListener('change', updateLivePreview))
...
function renderSections() {
  ...
  sections.forEach((s, i) => {
    ...
    const typeClass =
      { paragraph: 'para', quote: 'quote', image: 'image', video: 'video' }[
        s.type
      ] || 'para'
    const typeLabel =
      {
    ...
    let body = ''
    if (s.type === 'paragraph') {
      body = `<textarea class="sec-block__textarea" placeholder="Escreva aqui…">${
        s.text || ''
      }</textarea>`
    } else if (s.type === 'quote') {
      body = `<textarea class="sec-block__textarea" style="min-height:60px" placeholder="Citação…">${
        s.text || ''
      }</textarea>`
      ...
      body = `
        <div style="padding:0.75rem">
          <input type="text"
            style="background:hsl(38 25% 7%);border:1px solid hsl(38 25% 20%);border-radius:6px;color:var(--color-primary);font-size:0.85rem;outline:none;padding:0.5rem 0.75rem;width:100%;"
            placeholder="https://…" value="${s.url || ''}" />
        </div>`
    } else if (s.type === 'video') {
      body = `
        <div class="sec-block__video-area">
          <div class="sec-block__video-input">
            <span class="sec-block__video-icon">▶</span>
            <input type="text" placeholder="URL do YouTube…" value="${
              s.url || ''
            }" />
          </div>
        </div>`
    ...
    block.innerHTML = `
      <div class="sec-block__bar">
        ...
        <div class="sec-block__actions">
          <button class="sec-block__btn" data-move="-1" data-idx="${i}" title="Subir"  ${
            i === 0 ? 'disabled' : ''
          }>↑</button>
          <button class="sec-block__btn" data-move="1"  data-idx="${i}" title="Descer" ${
            i === sections.length - 1 ? 'disabled' : ''
          }>↓</button>
...
$('addImage').addEventListener('click', () => {
  sections.push({ type: 'image', url: '' })
  renderSections()
})
$('addVideo').addEventListener('click', () => {
  sections.push({ type: 'video', url: '' })
  renderSections()
})
...
setupUpload({
  areaId: 'bannerUploadArea',
  fileInputId: 'fBannerFile',
  previewId: 'bannerPreview',
  labelId: 'bannerUploadLabel',
  statusId: 'bannerStatus',
  hiddenId: 'fBanner',
  bucket: 'banners',
})
setupUpload({
  areaId: 'coverUploadArea',
  fileInputId: 'fCoverFile',
  previewId: 'coverPreview',
  labelId: 'coverUploadLabel',
  statusId: 'coverStatus',
  hiddenId: 'fCover',
  bucket: 'covers',
})

// =============================================================================
// SAVE — campo correto é "sections" (não "content")
// =============================================================================
$('saveReviewBtn').addEventListener('click', async () => {
  ...
  const payload = {
    ...
    sections: sections, // ← coluna correta
  }
...
// =============================================================================
// LOAD REVIEWS — seleciona todas as colunas existentes
// =============================================================================
async function loadReviews() {
  ...
  if (!reviews.length) {
    list.innerHTML = '<p class="loading">Nenhuma review ainda.</p>'
    return
  }
  ...
  list.innerHTML = reviews
    ...
    <div class="review-item ${
      r.id === currentReviewId ? 'active' : ''
    }" data-id="${r.id}">
      <img src="${
        r.cover_url || r.banner_url || ''
      }" alt="" onerror="this.style.visibility='hidden'" />
      <div class="review-item-info">
        <p class="review-item-title">${r.title || 'Sem título'}</p>
        <p class="review-item-slug" style="color:${
          r.category === 'anime' ? 'hsl(280 60% 62%)' : 'hsl(27 70% 52%)'
        }">
          ${r.category === 'anime' ? '🎎 Anime' : '🎮 Jogo'}${
            r.slug ? ' · ' + r.slug : ''
          }
        </p>
      </div>
...
function openReview(rev) {
  ...
  if (rev.banner_url) {
    bp.src = rev.banner_url
    bp.style.display = 'block'
    $('bannerUploadLabel').textContent = 'Banner carregado'
  } else {
    bp.style.display = 'none'
    $('bannerUploadLabel').textContent = '📁 Clique para selecionar'
  }

  const cp = $('coverPreview')
  if (rev.cover_url) {
    cp.src = rev.cover_url
    cp.style.display = 'block'
    $('coverUploadLabel').textContent = 'Capa carregada'
  } else {
    cp.style.display = 'none'
    $('coverUploadLabel').textContent = '📁 Clique para selecionar'
  }

  const radio = document.querySelector(
    `input[name="fCategory"][value="${rev.category || 'game'}"]`,
  )
  if (radio) radio.checked = true

  // Campo correto é "sections"
